...
public:
    ...
    {
        readScan(msg);  // reads into points
        // for (int i = 0; i < points.size(); i++)
        // {
        //     Point& p = points[i];
        //     if (isnan(p.r))
        //         cout << "readScan NaN range at " << i << endl;
        //     if (isnan(p.theta))
        //         cout << "readScan NaN theta at " << i << endl;
        // }
        ...
        int count = 0;
        computeJump(jump_table, prev_points);  // compute jump table for correspondence matching
        ...
        {
            transformPoints(points, curr_trans, transformed_points);  // use our current transform on our scanned points
    ...
    void readScan(const sensor_msgs::msg::LaserScan::ConstSharedPtr msg)
    {
        ...
        for (int i = 0; i < ranges.size(); ++i)
        {
            ...
            if (!isnan(range) && range >= range_min && range <= range_max)
            {
                ...
                if (!isnan(theta)) {
                    points.push_back(Point(range, theta));
                }
            }
        }
    }
